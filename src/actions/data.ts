...
import RecruitActivityModel from "@/lib/models/RecruitActivity";
...
export async function getRecruitActivity(): Promise<RecruitActivity[]> {
  await dbConnect();
  const docs = await RecruitActivityModel.find().sort({ year: -1 }).lean();
  return docs.map((e) => ({
    id: (e._id as object).toString(),
    title: e.title,
    category: e.category,
