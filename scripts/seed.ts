...
const MainActivitiesSchema = new mongoose.Schema({title: {type: String, required: true},description: {type: String, required: true}});
...
async function main() {
  ...
  const MainActivities = mongoose.model("MainActivities", MainActivitiesSchema);
  ...
  for (const a of [
    ...
    await MainActivities.create({ title: a.title, description: a.description });
    console.log(`  ✓ ${a.title}`);
  }
